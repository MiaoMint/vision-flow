...
import type { TextNodeData, WorkflowNodeData } from "@/components/nodes/types";
...
export function useCanvasCopyPaste({
  ...
  const copyNodes = useCallback(() => {
    ...
    if (selectedNodesList.length > 0) {
      ...
      // Clear system clipboard so paste will use internal copied nodes
  ...
  const pasteNodes = useCallback(async () => {
    ...
    try {
      ...
      if (clipboardText && clipboardText.trim()) {
        // Create a new text node with clipboard content
      ...
      // If clipboard access fails, fall through to node copy/paste logic
      console.error("Failed to access clipboard:", error);
    }

    // Original node copy/paste logic
